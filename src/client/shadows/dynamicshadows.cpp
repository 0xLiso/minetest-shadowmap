...
using m4f = irr::core::matrix4;
	...
	void DirectionalLight::createSplitMatrices(csmfrustum &subfrusta, const Camera *cam) {
		...
		v3f camPos = v3f(camPos2.X - cam->getOffset().X * BS,
				...
				camPos2.Z - cam->getOffset().Z *
								BS); //=
								     //cam->getCameraNode()->getPosition();
		...
		frustumCenter.X = (float)std::floor(
				frustumCenter.X); // clamp to texel increment
		frustumCenter.Y = (float)std::floor(
				frustumCenter.Y); // clamp to texel increment
		frustumCenter.Z = (float)std::floor(frustumCenter.Z);
		mLookAtInv.transformVect(frustumCenter);
		// probar radius multipliacdor en funcion del I, a menor I mas
		// multiplicador
		v3f eye_displacement = direction * radius*0.5;
		...
		subfrusta.csmProjOrthMat.buildProjectionMatrixOrthoLH(
				radius, radius, 1, subfrusta.length);
	}
...
void DirectionalLight::update_frustum(const Camera *cam, Client *client) {
	...
	/*float zFar = cam->getCameraNode()->getFarValue() > getMaxFarValue()
				 ...
				 : cam->getCameraNode()->getFarValue();
				     */
	float wanted_range = client->getEnv().getClientMap().getWantedRange() * BS;
	...
	for (int i = 0; i < nsplits; i++) {
		...
		// yes, i know this is wrong, but it´s just an approximation to test the
		// concept.
		client->getEnv().getClientMap().updateDrawListShadow(
				csm_frustum[i].position, getDirection(),
				csm_frustum[i].length);
	}
	should_update_map_shadow = true;
